...
class GridWaveEnvironment:
    ...
    def __init__(self, dt=0.01, grid_size=100, render_on=False, final_plot=False):
        """
    ...
    def set_wave_conditions(self, hs, tp, wave_dir_deg):
        ...
        self.waveload = WaveLoad(
            wave_amps, w, eps, wave_dir,
            config_file=self.vessel._config_file, interpolate=True
        )
    ...
    def step(self, action):
        ...
        tau_wf = self.waveload.first_order_loads(self.simulation_time, self.vessel.get_eta())
        tau_sv = self.waveload.second_order_loads(self.simulation_time, self.vessel.get_eta()[-1])
        tau_w = tau_wf + tau_sv
    ...
    def reset(self):
        ...
        eta_start = three2sixDOF(np.array([self.start_position[1], self.start_position[0], np.deg2rad(self.start_position[2])]))
