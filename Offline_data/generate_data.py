...
def run_simulation(wave_conditions, dt, simtime, start_pos):
    ...
    for step_count in range(int(simtime/dt)):
        ...
        tau = controller.get_tau(
            ...
            t=current_time,
            calculate_bias=False # Set to True to include the bias term in the control law
        )
    ...
    sim_data = {
        ...
        # },
        # The 'controller_tuning' key is commented out to reduce the size of the output file.
        # Uncomment if tuning parameters are needed in the output.
        'wave_conditions': wave_conditions,
    }
    return sim_data
...
def main():
    ...
    simtime  = 40    # Total simulation time (seconds)
    simtime         = 40      # Total simulation time (seconds)
    ...
    for traj in range(num_trajectories):
        ...
        wave_period = round(np.clip(wave_period, 6, 20),2)
